...
func main() {
    ...
    lis, err := net.Listen("tcp", fmt.Sprintf("localhost:%s", envs["PORT"]))
    if err != nil {
        log.Fatalf("Failed to listen: %v", err)
    }

    dbUrl := fmt.Sprintf("postgresql://%s:%s@localhost:5432/%s",
        envs["POSTGRES_USER"], envs["POSTGRES_PASSWORD"], envs["POSTGRES_DB"])
